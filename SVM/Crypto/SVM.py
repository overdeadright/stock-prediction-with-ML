...
def saveBalances(folder_name, size, history, stock):
    n = len(history)
    one_third = n // 3   # integer division to get the index of the 1/3 element
    two_thirds = 2 * n // 3   # integer division to get the index of the 2/3 element
    first_third = history[one_third]   # Get the 1/3 element of the list
    second_third = history[two_thirds]   # Get the 2/3 element of the list
    last_third = history[-1]   # Get the last element of the list

    # Check if the file exists, create it if not
    if not os.path.isfile(f'{folder_name}/balance_{stock}.csv'):
        with open(f'{folder_name}/balance_{stock}.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Size', 'Year 1', 'Year 2', 'Year 3'])

    # Write the balance values to the file
    with open(f'{folder_name}/balance_{stock}.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([size, first_third, second_third, last_third])
...
if __name__ == '__main__':    
    ...
    files = ["ADA-USD.csv", "ATOM-USD.csv", "BNB-USD.csv", 
             "BTC-USD.csv", "DOGE-USD.csv", "ETC-USD.csv", 
             "ETH-USD.csv", "LEO-USD.csv", "XMR-USD.csv", "XRP-USD.csv"]
