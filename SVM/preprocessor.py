...
def Preprocessor(file, start_date = "2020-01-02", end_date = "2022-12-30"):
    ...
    if start_date_index < 199 + max_size:
        raise ValueError(f"The specified date is not at least 299 rows below the header. It is {start_date_index}.")

    # Drop rows 199 rows below the date index (max_size left for size)
    drop_index = start_date_index - 199 - max_size 
    ...
    # Use a 20-period moving average and 2 standard deviations to calculate the Bollinger Bands
    ma20 = df['Close'].rolling(window=20).mean()
    std20 = df['Close'].rolling(window=20).std()

    # Calculate the upper and lower Bollinger Bands
    upper_band20 = ma20 + 2 * std20
    lower_band20 = ma20 - 2 * std20
    ...
    #df['Upper Band (20)'] = upper_band20
    #df['Lower Band (20)'] = lower_band20

    #df['Moving Average (50)'] = ma50
    #df['Moving Average (200)'] = ma200
    df['RSI'] = rsi
    df['ATR'] = atr
    df['OBV'] = obv
    #df['Stochastic Oscillator'] = stoch
    df['CMF'] = cmf
