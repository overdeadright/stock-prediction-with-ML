...
if not os.path.isfile("output_balance_all.csv"):
    with open("output_balance_all.csv", 'w', newline='') as output_file:
        writer = csv.writer(output_file)
        writer.writerow(['Stock', 'Year 1', 'Year 2', 'Year 3'])
...
for file in files:
    ...
    with open(filename, 'r') as f:
        ...
        year2 = []
        year3 = []
        # iterate over the rows (And add them up splitting into 3 parts)
        for row in reader:
        ...
        with open('output_balance_all.csv', 'a') as output_file:
            writer = csv.writer(output_file)
