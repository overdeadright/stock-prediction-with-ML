...
he = '''
def nthompson_sampling(reward_file, record_file, slots = 1, tokens = 1, tau = 0.1):
    rewards = []
    ...
    for i in range(num_trials):
        ...
        # Compute the softmax probabilities for each arm
        softmax_probs = np.exp(samples / tau) / np.sum(np.exp(samples / tau))
        
        # Choose the top k arms with the highest softmax probabilities as the selected arms for this 
        # trial
        chosen_arms = np.random.choice(n, slots, replace=False, p=softmax_probs)
...
if __name__ == "__main__":
    ...
    g = ["difference_AAPL.csv", "difference_AMZN.csv"]
    files = ["difference_GOOGL.csv", 
